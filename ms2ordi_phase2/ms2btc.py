...
python ms2btc.py export-burned-info-phase2
python ms2btc.py pair-phase2 --btc-hash=<btc-block-hash> --inscriptions-file=<xx.csv> --out-file=<xx.csv>
"""
...
PHASE2_BURNED_INFO_FILE = "ms_burned_phase2.csv"
...
def export_burned_info_phase2():
    ...
    phase1_token_ids = read_phase1_token_ids()
    ...
    phase2_token_ids = list(set(ms_ids) - set(phase1_token_ids))
    logger.info("phase2 token id count: {}", len(phase2_token_ids))

    phase2_btcs = contract.functions.getBTCs(phase2_token_ids).call()

    csv_writer = csv.DictWriter(Path(PHASE2_BURNED_INFO_FILE).open("w"), fieldnames=BURNED_INFO_FIELDNAMES)
    csv_writer.writeheader()
    counter = 1
    for token_id, btc in zip(phase2_token_ids, phase2_btcs):
    ...
    logger.info("output file: {}", PHASE2_BURNED_INFO_FILE)
...
def pair_phase2(
    ...
    # burned token ids
    csv_reader = csv.DictReader(Path(PHASE2_BURNED_INFO_FILE).open("r"), fieldnames=BURNED_INFO_FIELDNAMES)
    next(csv_reader)
    tokens = dict()
