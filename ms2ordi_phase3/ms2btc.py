...
python ms2btc.py export-burned-info-phase3
python ms2btc.py pair-phase3 --btc-hash=<btc-block-hash> --inscriptions-file=<xx.csv> --out-file=<xx.csv>
"""
...
BURNED_INFO_FILE = "ms_burned_phase12.csv"

PHASE3_BURNED_INFO_FILE = "ms_burned_phase3.csv"
...
def read_phase12_token_ids():
    reader = csv.DictReader(Path(BURNED_INFO_FILE).open(), BURNED_INFO_FIELDNAMES)
    next(reader)
    return list(int(i["token_id"]) for i in reader)


@app.command()
def export_burned_info_phase3():
    ms_ordi_abi = json.load(Path("MeshSpiritOrdi.json").open())
    w3 = Web3(Web3.HTTPProvider("https://mainnet.ethereumpow.org"))
    contract = w3.eth.contract(address=ms_ordi_abi["address"], abi=ms_ordi_abi["abi"])

    phase1_token_ids = read_phase12_token_ids()
    logger.info("phase1 token id count: {}", len(phase1_token_ids))

    ms_ids = contract.functions.meshSpiritIds().call()
    logger.info("burned count: {}", len(ms_ids))

    phase3_token_ids = list(set(ms_ids) - set(phase1_token_ids))
    logger.info("phase3 token id count: {}", len(phase3_token_ids))

    phase3_btcs = contract.functions.getBTCs(phase3_token_ids).call()

    csv_writer = csv.DictWriter(Path(PHASE3_BURNED_INFO_FILE).open("w"), fieldnames=BURNED_INFO_FIELDNAMES)
    csv_writer.writeheader()
    counter = 1
    for token_id, btc in zip(phase3_token_ids, phase3_btcs):
        logger.info("{}: tokenId[{}], btc[{}]", counter, token_id, btc)
    ...
    logger.info("output file: {}", PHASE3_BURNED_INFO_FILE)
...
def pair_phase3(
    ...
    # burned token ids
    csv_reader = csv.DictReader(Path(PHASE3_BURNED_INFO_FILE).open("r"), fieldnames=BURNED_INFO_FIELDNAMES)
    next(csv_reader)
    tokens = dict()
